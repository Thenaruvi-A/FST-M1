...
public class Activity5 {
   ...
   @BeforeClass
    ...
    public void smsTest() {
	   ...
	   driver.findElement(AppiumBy.xpath("//android.widget.EditText[@resource-id=\"ContactSearchField\"]")).sendKeys("8870004500");
	        // Press ENTER
...
// Assertion
	// wait.until(ExpectedConditions.elementToBeClickable(AppiumBy.id("message_text")));
	 WebElement message = driver.findElement(AppiumBy.xpath("//*[contains(@content-desc, 'Hello from Appium')]"));
     //String messageTextSent = driver.findElement(AppiumBy.id("message_text")).getText();
    //  Assert.assertEquals(message, "Hello from Appium");
	 String actualMessage = message.getAttribute("content-desc");
	Assert.assertTrue(actualMessage.contains("Hello from Appium"));
    }
   // Tear down method
    @AfterClass
